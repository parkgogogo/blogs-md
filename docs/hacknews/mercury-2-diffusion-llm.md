...
今天，我们正式推出 Mercury 2——这是世界上速度最快的推理语言模型，旨在让生产级 AI 实现即时响应的体验。
...
生产级 AI 不再是一个提示和一个答案的简单交互。它变成了循环：智能体（agents）、检索管道（retrieval pipelines）和后台大规模运行的提取任务。在这些循环中，延迟不会只出现一次——它会在每一步、每个用户、每次重试中累积叠加。

Yet current LLMs still share the same bottleneck: autoregressive, sequential decoding. One token at a time, left to right.

然而，当前的大语言模型仍然面临同一个瓶颈：自回归（autoregressive）的序列解码。一次只能生成一个 token，从左到右依次进行。
...
Mercury 2 不采用顺序解码。它通过并行优化（parallel refinement）生成响应，同时产生多个 token，并在少量步骤内收敛。不再像打字机一样逐字敲击，而更像编辑一次性修改整篇草稿。结果是：生成速度提升超过 5 倍，呈现出根本不同的速度曲线。
...
这种速度优势也改变了推理的权衡方式。如今，更高的智能意味着更多的测试时计算（test-time compute）——更长的推理链、更多的样本、更多的重试——而这直接以延迟和成本为代价。而基于扩散模型的推理（diffusion-based reasoning）则能在实时延迟预算内实现推理级别的质量。
...
- **速度：** 在 NVIDIA Blackwell GPU 上达到每秒 1,009 个 token
...
- **质量：** 与领先的 speed-optimized 模型竞争

- **Features:** tunable reasoning · 128K context · native tool use · schema-aligned JSON output
- **特性：** 可调节的推理深度（tunable reasoning）· 128K 上下文 · 原生工具使用（native tool use）· 符合 schema 的 JSON 输出
...
我们优化的是用户真正能感知到的速度：用户交互时刻的响应性——高并发下的 p95 延迟、一致的轮次间行为，以及系统繁忙时的稳定吞吐量。
...
> "Inception 的 Mercury 2 展示了当新模型架构遇上 NVIDIA AI 基础设施时所能实现的可能性。在 NVIDIA GPU 上突破每秒 1,000 个 token 的速度，凸显了我们平台驱动全谱系 AI 工作负载的性能、可扩展性和多功能性。"
...
智能体工作流（Agentic workflows）在每个任务中会串联数十个推理调用。降低每次调用的延迟不仅节省时间，更改变了你能负担得起的步骤数量，以及最终输出的质量。
...
> "我们现在正利用最新的 Mercury 模型来大规模智能优化营销活动执行。通过实时呈现洞察并动态增强投放，我们正在推动更强的性能、更高的效率，以及更具韧性的 AI 驱动广告生态系统。这一进展强化了我们对于自主化广告的承诺——智能系统持续优化执行，为客户带来可衡量的成果。"
...
> "我们一直在评估 Mercury 2，因为它拥有无与伦比的延迟表现和质量，这对于实时转录清理和交互式 HCI 应用尤其有价值。没有其他模型能达到 Mercury 提供的速度！"

> — Sahaj Garg, CTO & Co-Founder, Wispr Flow

> — Sahaj Garg，Wispr Flow 联合创始人兼 CTO

> "Mercury 2 is at least twice as fast as GPT-5.2, which is a game changer for us."

> "Mercury 2 至少比 GPT-5.2 快两倍，这对我们来说是游戏规则的改变者。"

> — Suchintan Singh, CTO & Co-Founder, Skyvern

> — Suchintan Singh，Skyvern 联合创始人兼 CTO
...
语音界面在 AI 中有着最严格的延迟预算。Mercury 2 让推理级别的质量在自然语音节奏下成为可能。
...
> "我们打造逼真的 AI 视频化身，与真人进行实时对话，因此低延迟不是锦上添花，而是一切的根本。Mercury 2 在我们的语音技术栈中是一个重大突破：快速、一致的文本生成让整个体验保持自然和人性化。"

> — Max Sapo, CEO & Co-Founder, Happyverse AI

> — Max Sapo，Happyverse AI 联合创始人兼 CEO

> "Mercury 2 quality is excellent, and the model's low latency enables more responsive voice agents."

> "Mercury 2 的质量非常出色，模型的低延迟让语音智能体的响应性更强。"
...
多跳检索（multi-hop retrieval）、重排序（reranking）和摘要的延迟会快速叠加。Mercury 2 让你在搜索循环中加入推理能力，而不会突破延迟预算。
...
> "我们与 Inception 的合作让我们搜索产品的实时 AI 成为可能。每个 SearchBlox 客户——涵盖客户支持、合规、风险、分析和电商——都能从其所有数据中受益于亚秒级智能。"
...
- **[Request Early Access](https://www.inceptionlabs.ai/early-access)**
- **[申请早期访问](https://www.inceptionlabs.ai/early-access)**

- **[Try Mercury 2 in Chat](https://chat.inceptionlabs.ai)**
- **[在 Chat 中体验 Mercury 2](https://chat.inceptionlabs.ai)**
...
如果你正在进行企业级评估，我们将与你合作，针对工作负载适配、评估设计和预期服务约束下的性能验证提供支持。

**Mercury 2 is live. Welcome to diffusion.**

**Mercury 2 已正式上线。欢迎来到扩散模型的时代。**

---

*Author: Stefano Ermon, CEO, Inception Labs*

*作者：Stefano Ermon，Inception Labs CEO*
