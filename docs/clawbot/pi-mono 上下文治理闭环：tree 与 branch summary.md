...
很多人第一次用带 `/tree` 的 Agent，会以为它是“回到过去”。

但真正让人崩溃的瞬间往往是这样的：

- 你在分支 A 上试了一套方案，走到一半发现不对
- 你跳回到更早的一个点，开分支 B 继续
- 你以为“历史都在”，但你脑子里的那段探索路径（为什么不行、试过什么）突然没了

这时候你会产生一种很强的饥渴感：

> 我想把刚才那段探索“吃掉”，变成我现在分支的营养。

你说的英文状态 **devour** 我理解就是这个：不是简单读一遍，而是要把经验吞进系统里，变成可用上下文。

`pi-mono` 的做法很聪明：它把“长会话压缩”和“分支切换”当成一个系统来做——**上下文治理闭环**。

### 核心理念（一句话）

`pi` 把“对话历史”当成一棵树来管理：

> 你可以随时换路走，但每次离开一条路，会在路口立一块路牌（branch summary），让你以后回来或切换时不迷路。

### 术语地图（先读这段）

- **session tree（会话树）**：同一个会话文件里，消息不是一条直线，而是可以分叉。
- **leaf（叶子/当前指针）**：你现在所在的那条分支末端。
- **/tree（树导航）**：把“当前指针”移动到树上任意节点继续。
- **branch summary（分支摘要/路牌）**：当你离开某条分支时，把那条分支的探索总结成一段“可携带经验”。
- **compaction（压缩/总结页+书签）**：解决“太长装不下”的问题。
